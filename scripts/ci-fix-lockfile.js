#!/usr/bin/env node
// Simple CI lockfile checker for package.json vs package-lock.json
...
function compareDeps(depMap) {
  ...
  for (const [name, range] of Object.entries(depMap || {})) {
    ...
    // heuristic: if expected is empty (git/other), skip strict check
    if (!expected || expected.startsWith('git+') || expected.includes(':')) continue;
    // if exact match
    if (locked === expected) continue;
    // if package.json had ^ or ~ and major matches, accept
